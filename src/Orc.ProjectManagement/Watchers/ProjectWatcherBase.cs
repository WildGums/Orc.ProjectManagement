...
public abstract class ProjectWatcherBase
{
    ...
    private void InitSubscriptions()
    {
        ...
        {
            subscribeMethod.Invoke(this, Array.Empty<object>());
        }
    }

    private void SubscribeOnLoading()
    {
        _projectManager.ProjectLoadingAsync += async (_, e) => await OnLoadingInternalAsync(e).ConfigureAwait(false);
    }

    private void SubscribeOnLoadingFailed()
    {
        _projectManager.ProjectLoadingFailedAsync += async (_, e) => await OnLoadingFailedInternalAsync(e.Location ?? string.Empty, e.Exception, e.ValidationContext).ConfigureAwait(false);
    }

    private void SubscribeOnLoadingCanceled()
    {
        _projectManager.ProjectLoadingCanceledAsync += async (_, e) => await OnLoadingCanceledInternalAsync(e.Location ?? string.Empty).ConfigureAwait(false);
    }

    private void SubscribeOnLoaded()
    {
        _projectManager.ProjectLoadedAsync += async (_, e) => await OnLoadedInternalAsync(e.Project).ConfigureAwait(false);
    }

    private void SubscribeOnSaving()
    {
        _projectManager.ProjectSavingAsync += async (_, e) => await OnSavingInternalAsync(e).ConfigureAwait(false);
    }

    private void SubscribeOnSavingCanceled()
    {
        _projectManager.ProjectSavingCanceledAsync += async (_, e) => await OnSavingCanceledInternalAsync(e.Project).ConfigureAwait(false);
    }

    private void SubscribeOnSavingFailed()
    {
        _projectManager.ProjectSavingFailedAsync += async (_, e) =>
        {
    ...
    {
        _projectManager.ProjectSavedAsync += async (_, e) => await OnSavedInternalAsync(e.Project).ConfigureAwait(false);
    }

    private void SubscribeOnClosing()
    {
        _projectManager.ProjectClosingAsync += async (_, e) => await OnClosingInternalAsync(e).ConfigureAwait(false);
    }

    private void SubscribeOnClosed()
    {
        _projectManager.ProjectClosedAsync += async (_, e) => await OnClosedInternalAsync(e.Project).ConfigureAwait(false);
    }

    private void SubscribeOnClosingCanceled()
    {
        _projectManager.ProjectClosingCanceledAsync += async (_, e) => await OnClosingCanceledInternalAsync(e.Project).ConfigureAwait(false);
    }

    private void SubscribeOnActivated()
    {
        _projectManager.ProjectActivatedAsync += async (_, e) => await OnActivatedInternalAsync(e.OldProject, e.NewProject).ConfigureAwait(false);
    }

    private void SubscribeOnActivationFailed()
    {
        _projectManager.ProjectActivationFailedAsync += async (_, e) => await OnActivationFailedInternalAsync(e.Project, e.Exception).ConfigureAwait(false);
    }

    private void SubscribeOnActivationCanceled()
    {
        _projectManager.ProjectActivationCanceledAsync += async (_, e) => await OnActivationCanceledInternalAsync(e.Project).ConfigureAwait(false);
    }

    private void SubscribeOnActivation()
    {
        _projectManager.ProjectActivationAsync += async (_, e) => await OnActivationInternalAsync(e).ConfigureAwait(false);
    }

    private void SubscribeOnRefreshRequired()
    {
        _projectManager.ProjectRefreshRequiredAsync += async (_, e) => await OnRefreshRequiredInternalAsync(e).ConfigureAwait(false);
    }

    private void SubscribeOnRefreshed()
    {
        _projectManager.ProjectRefreshedAsync += async (_, e) => await OnRefreshedInternalAsync(e.Project).ConfigureAwait(false);
    }

    private void SubscribeOnRefreshingFailed()
    {
        _projectManager.ProjectRefreshingFailedAsync += async (_, e) =>
        {
            var project = e.Project;
            if (project is null)
            {
    ...
    {
        _projectManager.ProjectRefreshingCanceledAsync += async (_, e) => await OnRefreshingCanceledInternalAsync(e.Project).ConfigureAwait(false);
    }

    private void SubscribeOnRefreshing()
    {
        _projectManager.ProjectRefreshingAsync += async (_, e) => await OnRefreshingInternalAsync(e).ConfigureAwait(false);
    }
