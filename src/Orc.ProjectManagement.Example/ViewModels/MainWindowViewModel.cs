...
public class MainWindowViewModel : ViewModelBase
{
    ...
    public TaskCommand LoadProject { get; }
    ...
    private async Task OnLoadProjectExecuteAsync()
    {
        ...
        if (result.Result)
        {
            ...
            {
                Log.Warning("Can't load project with no file");

                return;
            }

            await _projectManager.LoadAsync(fileName).ConfigureAwait(false);
        }
    }

    public TaskCommand RefreshProject { get; }
    ...
    public TaskCommand SaveProject { get; }
    ...
    public TaskCommand SaveProjectAs { get; }
    ...
    public Command CloseProject { get; }
    ...
    {
        var location = _projectManager.ActiveProject?.Location;
        if (string.IsNullOrWhiteSpace(location))
        {
            Log.Warning("Can't open project without file");
